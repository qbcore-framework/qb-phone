...
function QBPhone.AddMentionedTweet(citizenid, TweetData)
    if not MentionedTweets[citizenid] then
...
QBCore.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    ...
    if Player then
        ...
        local TWData = MySQL.Sync.fetchAll('SELECT * FROM phone_tweets')
        if TWData and next(TWData) then
            PhoneData.Tweets = TWData
            Tweets = TWData
        end
...
RegisterNetEvent('qb-phone:server:sendNewMail', function(mailData)
    ...
    SetTimeout(200, function()
        local mails = MySQL.Sync.fetchAll('SELECT * FROM player_mails WHERE citizenid = ? ORDER BY `date` DESC', {Player.PlayerData.citizenid})
        if mails[1] then
            for k, v in pairs(mails) do
                if mails[k].button then
                    mails[k].button = json.decode(mails[k].button)
...
RegisterNetEvent('qb-phone:server:DeleteTweet', function(tweetId)
    ...
    if Data == Player.PlayerData.citizenid then
        MySQL.Async.fetchAll('DELETE FROM phone_tweets WHERE tweetId = ?', {TID})
        delete = true
    end
    if delete then
        delete = not delete
        for k, v in pairs(Tweets) do
            if Tweets[k].tweetId == TID then
                Tweets[k] = nil
            end
        end
        TriggerClientEvent('qb-phone:client:UpdateTweets', -1, Tweets)
    end
end)
...
RegisterNetEvent('qb-phone:server:UpdateTweets', function(NewTweets, TweetData)
    ...
    MySQL.Async.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
        TweetData.citizenid,
        TweetData.firstName,
        TweetData.lastName,
        TweetData.message,
...
RegisterNetEvent('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    if result[1] then
        ...
        if reciever then
            ...
            MySQL.Async.execute('UPDATE players SET money = ? WHERE citizenid = ?', {json.encode(moneyInfo), result[1].citizenid})
...
RegisterNetEvent('qb-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, oldIban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute('UPDATE player_contacts SET name = ?, number = ?, iban = ? WHERE citizenid = ? AND name = ? AND number = ?', {newName, newNumber, newIban, Player.PlayerData.citizenid, oldName, oldNumber})
end)

RegisterNetEvent('qb-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.Async.execute('DELETE FROM player_contacts WHERE name = ? AND number = ? AND citizenid = ?', {Name, Number, Player.PlayerData.citizenid})
end)
...
RegisterNetEvent('qb-phone:server:SaveMetaData', function(MData)
    ...
    MySQL.Async.execute('UPDATE players SET metadata = ? WHERE citizenid = ?', {json.encode(MetaData), Player.PlayerData.citizenid})
