    ...
    if Player then
        ...
        if garageresult[1] then
            for _, v in pairs(garageresult) do
                ...
                if (QBCore.Shared.Vehicles[vehicleModel]) and (Config.Garages[v.garage]) then
...
RegisterNetEvent('qb-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.update('UPDATE player_contacts SET name = ?, number = ?, iban = ? WHERE citizenid = ? AND name = ? AND number = ?', {
        newName,
        newNumber,
        newIban,
        Player.PlayerData.citizenid,
